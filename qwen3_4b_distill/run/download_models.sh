#!/usr/bin/env bash
# 下载课题所需模型到 $MODELS。自带环境/路径声明（source env.sh），无需自行 export/mkdir 路径。
# 优先 modelscope；重试直到无 .incomplete 残留（网络差会留半截文件）；已完整则跳过。
# 全部输出自动落 $LOGS/run/download_models.log。
# 用法：
#   nohup bash download_models.sh >/dev/null 2>&1 &   # 后台下 T1 必需 4 个(~40G)；已完整的跳过，只补未下全的
