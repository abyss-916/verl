#!/usr/bin/env bash
# 三法下游 SFT→eval 满配对比 | OlymMATH-hard(held-out) 100题 | n=4 | 每法两卡分片、三法串行
#   对比对象：base(M1) vs standard_cot / reverse / question_aug。
#   模型用 peft 合并后的 $CKPT/sft_<法>_merged（verl model_merger 不折叠 LoRA，务必用 _merged）。
#
# 用法（服务器，先 conda activate verl；先看 nvidia-smi 确认两卡够空再起）：
...
#
# ⚠️ 每法两卡各跑一片=占满两张卡，故三法【串行】；单法约 7h、三法约 22h（n=4、思考~22K token/条）。
# ⚠️ 必须 setsid，不能只 nohup —— vLLM/torchrun 孙进程自带信号处理器，关终端 SIGHUP 会杀穿 nohup。
set -uo pipefail   # 故意不加 -e：某一法/某片失败要跳过、不拖垮其余两法
source "$(dirname "$0")/env.sh"

DATA_PARQUET=${DATA_PARQUET:-$EVAL_DIR/test.parquet}   # held-out OlymMATH，绝不用训练集
N=${N:-4}
...
for M in $METHODS; do
  ...
  rm -rf "$S0" "$S1" "$OUT"      # 清旧分片/合并结果，防脏数据混入
