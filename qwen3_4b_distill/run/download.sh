#!/usr/bin/env bash
# 下载课题所需模型到 $MODELS 并接入数据集（→ verl parquet）到 $DATA。自带 source env.sh；重试至无 .incomplete，已完整则跳过。
# 用法：
#   bash run/download.sh          # T1 必需模型(~40G)
#   bash run/download.sh t3       # 追加强度轴 T3(14B+32B-AWQ,~46G)
#   bash run/download.sh data     # 只接入数据集（SEED + EVAL → parquet）
#   bash run/download.sh all      # 模型全档 + 数据集
#   HF=1 bash run/download.sh     # 改用 HF 镜像(hf-mirror)
set -euo pipefail

# ---- 环境与路径 ----
source "$(dirname "$0")/env.sh"                         # 声明 $MODELS/$LOGS/$HF_ENDPOINT 并创建 /data 下各目录
mkdir -p "$LOGS/run" "$MODELS"
exec > >(tee -a "$LOGS/run/download.log") 2>&1         # 全部输出落日志
...
T3=(Qwen/Qwen3-14B Qwen/Qwen3-32B-AWQ)                                                    # 强度轴 teacher
DL_DATA=0
case "${1:-t1}" in
  t1)   LIST=("${T1[@]}") ;;
  t3)   LIST=("${T3[@]}") ;;
...
if [ "$DL_DATA" = "1" ]; then
  ...
  # SEED（训练/蒸馏种子，默认 MATH-lighteval）
